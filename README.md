...
На странице релиза лежат два экземпляра приложения:

- **`zapret-web-panel.exe`** — обычная сборка. Берите её.
- **`zapret-web-panel-lite.exe`** — то же самое приложение, из которого при упаковке исключена пара неиспользуемых модулей стандартной библиотеки. Меньше примерно на мегабайт, ведёт себя идентично.

Больше между ними ничего не отличается. В частности, **ни одна из сборок не сжата UPX** — заметки к релизу v1.9.0 утверждали обратное про lite-версию, но по факту упаковщик тогда не был установлен и шаг сжатия молча пропускался. Так что опасаться за lite-сборку из-за UPX не нужно: перед антивирусами обе версии выглядят одинаково.
